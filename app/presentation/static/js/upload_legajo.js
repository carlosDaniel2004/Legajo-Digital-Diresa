// upload_legajo.js
let listaPersonalCompleta = []; // Guardar la lista completa para filtrado

// Variable global para almacenar si hay personal preseleccionado
let personalPreseleccionado = null;

document.addEventListener('DOMContentLoaded', function() {
    // Obtener datos del personal preseleccionado del HTML si existen
    const alertaPreseleccionada = document.querySelector('.alert-info');
    if (alertaPreseleccionada && alertaPreseleccionada.textContent.includes('Personal preseleccionado')) {
        // El personal viene preseleccionado desde el servidor
        // Cargar lista y luego seleccionar automáticamente
        personalPreseleccionado = true;
    }
...
function cargarListaPersonal() {
    const urlAPI = '/pdf/api/personal-list';
...
function actualizarSelectPersonal(listaPersonal) {
    ...
    // Si hay un personal preseleccionado, seleccionarlo automáticamente
    if (window.idPersonalPreseleccionado) {
        select.value = window.idPersonalPreseleccionado;
        actualizarInfoPersonal();
        console.log('✓ Personal preseleccionado ID:', window.idPersonalPreseleccionado);
    }
}
...
function actualizarInfoPersonal() {
    ...
    const urlEstructura = `/pdf/api/estructura-personal/${id_personal}`;
    console.log('Cargando estructura desde:', urlEstructura);
    fetch(urlEstructura)
        .then(response => {
